...
env_param = {
    ...
    'lim_em_metros': 1.2,
    'pose_inic': (0.2, 0.2, 0),
    'desvio_pose_inic': (0.05, 0.05, np.pi/6),
...
param_graf = {
    ...
    'var_adic': ('v', 'phi'),
    'nomes_var_adic_tex': (r'$\dot{v}$', r'$\dot{\phi}$'),
...
def avaliar_individuo_multi(ind, num_episodios=param_pg['n_episodios'], num_entradas=param_pg['n_entradas'],
    ...
    for episodio in range(num_episodios):
        ...
        if pose is None:
            ...
            observacao = ambiente.reset(pose)
        while not termino:
            var_aux['n_dt'] += 1
            tempo_ep += 1
    ...
    if graficos:
        ...
        plotar_trajetoria(info, 50)
    if not mujoco and video:
        ambiente.close()
    return aptidao_media, info
...
def plotar_arvore(ind):
    ...
    nx.draw_networkx_labels(g, pos, labels, font_color='white', font_size=16)
    plt.show()
...
def plotar_multiplas_trajetorias(ind, rowcol: tuple, pontos, *args):
    funcao_de_controle_vel = toolbox.compilar_individuo(ind[0])
    funcao_de_controle_phi = toolbox.compilar_individuo(ind[1])
    ambiente = gym.make('Carrinho-v0', **env_param)
    figura, eixo = plt.subplots(nrows=rowcol[0], ncols=rowcol[1], sharex='col', sharey='row')
    cores = ['b', 'g', 'r', 'm', 'c']
    for i in range(rowcol[0]):
        for j in range(rowcol[1]):
            termino, observacao = False, ambiente.reset(args[i+j])
            custo_acumulado = 0
            while not termino:
                resultado_vel = funcao_de_controle_vel(*tuple(observacao))
                resultado_phi = funcao_de_controle_phi(*tuple(observacao))
                ...
                custo_acumulado += calc_custo_acum(recomp=recompensa)
                custo = calc_custo(custo_acum=custo_acumulado)
            eixo[i][j].plot('x', 'y', data=info, lw=1, ls='-')
            for k in np.arange(info['dts'][0], info['dts'][-1], int(info['dts'][-1] / pontos)):
                x, y, theta = info['x'][k], info['y'][k], info['theta'][k]
                eixo[i][j].annotate("",
                                    xy=(x + 0.02 * np.cos(theta), y + 0.02 * np.sin(theta)), xycoords='data',
                                    xytext=(x, y), textcoords='data',
                                    arrowprops=dict(arrowstyle="-|>",
                                                    connectionstyle="arc3",
                                                    mutation_scale=20))
                eixo[i][j].annotate(str(round(custo_acumulado)), xy=(0.1, 0.9), xycoords="axes fraction")
            eixo[i][j].grid(True)
            if i == rowcol[0] - 1:
                eixo[i][j].set_xlabel(r'$x$')
            if (i*rowcol[1] + j) % rowcol[1] == 0:
                eixo[i][j].set_ylabel(r'$y$')
            eixo[i][j].set_xlim([0, env_param['lim_em_metros']])
            eixo[i][j].set_ylim([0, env_param['lim_em_metros']])
    figura.tight_layout()
...
def plotar_hits(data=None, nrows=param_graf['hist_n_linhas'], ncols=param_graf['hist_n_cols'],
    ...
    for k in range(ncols):
        for l in range(nrows):
            N, bins, patches = eixo[l][k].hist(dados[graf[n]], bins=np.arange(bins_min, bins_max, bins_step))
            for thisbin, thispatch in zip(bins, patches):
                color = plt.cm.viridis(norm(thisbin))
                thispatch.set_facecolor(color)
            eixo[l][k].set_title('Geração ' + str(graf[n]))
            eixo[l][k].set_ylabel('Ocorrencias') if k is 0 else None
            eixo[l][k].set_xlabel('Aptidão') if l is (nrows-1) else None
            n += 1
...
def avaliar_melhor_hdf(hdf):
    best = -100000
    for ind in hdf:
        aptidao, _= avaliar_individuo_multi(ind, num_episodios=100)
...
#
# plotar_multiplas_trajetorias(store_dict['hallsdafama'][0], (3, 3), 5,
#                              (0.25, 0.15, 0), (0.25, 0.2, 0), (0.25, 0.25, 0),
#                              (0.2, 0.15, 0), (0.2, 0.2, 0), (0.2, 0.25, 0),
#                              (0.15, 0.15, 0), (0.15, 0.2, 0), (0.15, 0.25, 0))
#
# plotar_multiplas_trajetorias(store_dict['hallsdafama'][0], (3, 3), 5,
#                              (0.25, 0.15, np.pi/6), (0.25, 0.2, np.pi/6), (0.25, 0.25, np.pi/6),
#                              (0.2, 0.15, np.pi/6), (0.2, 0.2, np.pi/6), (0.2, 0.25, np.pi/6),
#                              (0.15, 0.15, np.pi/6), (0.15, 0.2, np.pi/6), (0.15, 0.25, np.pi/6))
#
# plotar_multiplas_trajetorias(store_dict['hallsdafama'][0], (3, 3), 5,
#                              (0.25, 0.15, -np.pi/6), (0.25, 0.2, -np.pi/6), (0.25, 0.25, -np.pi/6),
#                              (0.2, 0.15, -np.pi/6), (0.2, 0.2, -np.pi/6), (0.2, 0.25, -np.pi/6),
...
# plotar_arvore(store_dict['hallsdafama'][0][1])


# hits = []
# dif_angs = []
# for x in np.arange(0.15, 0.25, 0.01):
#     for y in np.arange(0.15, 0.25, 0.01):
#         for theta in np.arange(-np.pi/3, np.pi/3, 0.21):
#             apt, info = avaliar_individuo_multi(store_dict['hallsdafama'][0], pose=(x, y, theta))
#             if apt > 0:
#                 print("Sucess!")
#                 hits.append(True)
#                 dif_angs.append(info['dif_ang'])
#             else:
#                 print("Fail!")
#                 hits.append(False)
#
# print('Sucess rate: ', (sum(hits) / len(hits)) * 100, '%')
# clean = [x for x in dif_angs if x != None]
# print('Media ângulos: ', np.average(clean))
# print('Done')