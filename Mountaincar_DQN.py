...
model = DQN(MlpPolicy, env, verbose=1, prioritized_replay=True, param_noise=True)
...
if modo is 'y':
    ...
    print("Tempo de Treinamento (s): ", time.time() - tinit)
    sys.stdout.close()

    results_plotter.plot_results([log_dir], time_steps, results_plotter.X_TIMESTEPS, "DQN MountainCar")
    plt.show()
    ...
    act, observ, rew, tsteps = np.array([]), np.ndarray(shape=(2, 0)), np.array([]), np.array([])
    while not done:
        action, _states = model.predict(obs)
        obs, rewards, done, info = env.step(action)
        act = np.append(act, action)
        observ = np.hstack((observ, obs.reshape((2, 1))))
